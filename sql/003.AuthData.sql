...
insert into object(name) values ('watering system');
...
insert into user_permission(user_id,permission_id,object_id) values((select id from users where login_identifier = 'admin@example.com'),(select id from permissions where permission = 'admin'),(select id from object where name = 'watering system'));